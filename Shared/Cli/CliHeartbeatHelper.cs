...
{
    public static class CliHeartbeatHelper
    {
        public static bool TryCreateHeartbeatTemplate(JObject raw, out CliHeartbeatTemplate template)
        {
            template = null;
            if (raw == null)
                return false;
            ...
            template = new CliHeartbeatTemplate(interval, opTemplate, opType);
            return true;
        }
        ...
        private static JToken FindHeartbeatToken(JObject root)
        {
            ...
            {
                var token = CliJsonUtilities.SafeSelectToken(root, path);
                if (token != null)
                    return token;
            }
    ...
    public sealed class CliHeartbeatTemplate
    {
        public CliHeartbeatTemplate(TimeSpan interval, JObject opTemplate, string operationType)
        {
            Interval = interval;
            OpTemplate = opTemplate ?? new JObject();
        ...
        public CliHeartbeatInfo ToInfo() => new(Interval, OperationType);
    }
}
